...
resource "google_dialogflow_cx_tool" "open_api_tool" {
  ...
  open_api_spec {
    ...
    # Use the jsonencode function to safely build the JSON string
    # and reference the Cloud Run service's URI.
    text_schema = jsonencode({
      openapi = "3.0.0"
      info = {
      ...
      servers = [
        {
          # Dynamically set the URL from the Cloud Run service output
