...
def image_to_video_tab():
    ...
    with col1_img_prompt:
        ...
        if st.button("🚀 Refine Image Prompt with Gemini", use_container_width=True, type="primary"):
            if st.session_state.raw_image_user_prompt: 
                ...
                st.session_state.image_to_video_output_bytes = None # Clear previous video 
    ...
    # New: Imagen model selection
    st.session_state.imagen_model_choice = st.radio(
        "Choose Imagen Model:",
        ("Imagen 3", "Imagen 4"),
        key="imagen_model_selector",
        on_change=lambda: (
            setattr(st.session_state, 'image_bytes', None),
            setattr(st.session_state, 'image_to_video_output_bytes', None)
        )
    )
    ...
    if st.session_state.image_bytes:
        st.image(st.session_state.image_bytes, caption=f"Generated by Imagen ({st.session_state.imagen_model_choice})", use_container_width=True)
    else:
    ...
    if st.session_state.animate_image_prompt_option == "With prompt":
        ...
        with col1_vid_prompt:
            st.session_state.raw_veo_user_prompt = st.text_area(
                ...
                on_change=lambda: setattr(st.session_state, 'image_to_video_output_bytes', None), # Clear video on prompt change
    ...
    if st.session_state.image_to_video_output_bytes:
            ...
            st.video(st.session_state.image_to_video_output_bytes)
    else:
...
def text_to_video_tab():
    ...
    with col1_text_vid_prompt:
            st.session_state.raw_text_to_video_user_prompt = st.text_area(
                ...
                key="raw_text_to_video_user_prompt_input",
                value=st.session_state.raw_text_to_video_user_prompt,
                on_change=lambda: setattr(st.session_state, 'text_to_video_veo2_output_bytes', None), # Clear video on prompt change
                help="Describe the entire video, including characters, actions, environment, and camera movements."
            )
            ...
            if st.button("🚀 Refine Video Prompt with Gemini", use_container_width=True, type="primary", key="refine_text_video_prompt_button"):
                if st.session_state.raw_text_to_video_user_prompt:
                    with st.spinner("Gemini is refining your video prompt..."):
                        ...
                        if refined_text_to_video_prompt:
                            st.session_state.text_to_video_refined_prompt = refined_text_to_video_prompt
                    st.session_state.text_to_video_veo2_output_bytes = None # Clear previous video
                else:
                    st.warning("Please enter an initial idea for the video to refine.")
    ...
    if st.button("🎬 Generate Video from Text", use_container_width=True, 
        ...
        if not final_text_to_video_prompt:
            ...
            with st.spinner("Veo is generating your video from text... This can take several minutes."):
                st.session_state.text_to_video_veo2_output_bytes = generate_video_with_veo("text", None, final_text_to_video_prompt)

    if st.session_state.text_to_video_veo2_output_bytes:
        st.subheader("Your Generated Video (Veo2)")
        st.video(st.session_state.text_to_video_veo2_output_bytes)
    else:
        st.info("Your generated video (Veo2) will appear here after Veo finishes processing.")



    st.header("Text to Video Generation with Veo3")
    st.markdown("Describe the entire video you envision. Gemini can help you refine your prompt for better results.")

    col1_text_vid2_prompt, col2_text_vid2_prompt = st.columns(2)

    with col1_text_vid2_prompt:
        st.session_state.raw_text_to_veo3_user_prompt = st.text_area(
            "Enter your idea for the video:", 
            height=150, 
            key="raw_text_to_veo3_user_prompt_input",
            value=st.session_state.raw_text_to_veo3_user_prompt,
            on_change=lambda: setattr(st.session_state, 'text_to_video_veo3_output_bytes', None), # Clear video on prompt change
            help="Describe the entire video, including characters, actions, environment, and camera movements."
        )

        if st.button("🚀 Refine Video Prompt with Gemini", use_container_width=True, type="primary", key="refine_text_veo3_prompt_button"):
            if st.session_state.raw_text_to_veo3_user_prompt:
                with st.spinner("Gemini is refining your video prompt..."):
                    refined_text_to_video_prompt = refine_prompt_with_gemini(st.session_state.raw_text_to_veo3_user_prompt, for_video=True)
                    if refined_text_to_video_prompt:
                        st.session_state.text_to_veo3_refined_prompt = refined_text_to_video_prompt
                st.session_state.text_to_video_veo3_output_bytes = None # Clear previous video
            else:
                st.warning("Please enter an initial idea for the video to refine.")

    with col2_text_vid2_prompt:
        if st.session_state.text_to_veo3_refined_prompt:
            st.markdown("**Gemini's Refined Video Prompt:**")
            st.info(st.session_state.text_to_veo3_refined_prompt)
        elif st.session_state.raw_text_to_veo3_user_prompt:
            st.markdown("**Your Video Prompt:**")
            st.info(st.session_state.raw_text_to_veo3_user_prompt)
        else:
            st.info("Your refined video prompt will appear here.")
    
    final_text_to_video_prompt_veo3 = ( # Renamed to avoid conflict with Veo2 prompt
        st.session_state.text_to_veo3_refined_prompt
        or st.session_state.raw_text_to_veo3_user_prompt
    )

    st.markdown("---")

    if st.button("🎬 Generate Video from Text", use_container_width=True, 
                disabled=not final_text_to_video_prompt_veo3, key="generate_text_veo3_button"):
        if not final_text_to_video_prompt_veo3:
            st.warning("Please provide a prompt for video generation.")
        else:
            with st.spinner("Veo is generating your video from text... This can take several minutes."):
                st.session_state.text_to_video_veo3_output_bytes = generate_video_with_veo3("text", None,final_text_to_video_prompt_veo3)

    if st.session_state.text_to_video_veo3_output_bytes:
        st.subheader("Your Generated Video (Veo3)")
        st.video(st.session_state.text_to_video_veo3_output_bytes)
    else:
        st.info("Your generated video (Veo3) will appear here after Veo finishes processing.")
...
def long_video_tab():
    ...
    st.subheader("Select Veo Model")
    # New: Model selection for long video generation
    st.session_state.long_video_model_choice = st.radio(
        "Choose Veo model for segments:",
        ("Veo2", "Veo3"),
        key="long_video_model_selector",
        on_change=lambda: (
            setattr(st.session_state, 'long_video_final_video', None),
            setattr(st.session_state, 'long_video_segment_bytes', [None] * len(st.session_state.long_video_segment_bytes))
        )
    )
    st.markdown("---")
    ...
    for i in range(num_segments):
        ...
        with col_prompt:
            st.session_state.long_video_raw_segment_prompts[i] = st.text_area(
                ...
                # On any change in the prompt, clear the generated video bytes for this segment and the final video
                on_change=lambda current_idx=i: (
                    st.session_state.long_video_segment_bytes.__setitem__(current_idx, None),
                    setattr(st.session_state, 'long_video_final_video', None)
                ),
                ...
                if st.session_state.long_video_raw_segment_prompts[i]:
                    with st.spinner(f"Refining prompt for Segment {i+1}..."):
                        ...
                        if refined_segment_prompt:
                            ...
                            st.session_state.long_video_segment_bytes[i] = None # Clear video for this segment
                            st.session_state.long_video_final_video = None # Clear final video
                    st.rerun() 
                else:
        ...
        # Display individual segment video only if it exists
    ...
    with col_add_remove[0]:
        if st.button("➕ Add Another Segment", use_container_width=True):
            ...
            st.session_state.long_video_refined_segment_prompts.append("") 
            st.session_state.long_video_segment_bytes.append(None) # Add a None for the new segment's video
            st.session_state.long_video_final_video = None # Clear final video
            st.session_state.long_video_segments = [] # Clear temporary paths
            st.rerun() 
    with col_add_remove[1]:
        if len(st.session_state.long_video_raw_segment_prompts) > 1:
            if st.button("➖ Remove Last Segment", use_container_width=True):
                ...
                st.session_state.long_video_segment_bytes.pop() # Remove the last segment's video
                st.session_state.long_video_final_video = None # Clear final video
                st.session_state.long_video_segments = [] # Clear temporary paths
                st.rerun()
    ...
    if st.button("🚀 Generate All Video Segments", use_container_width=True, type="primary", key="generate_all_segments_button"):
        ...
        if all_prompts_valid:
            st.session_state.long_video_segments = [] # Paths for MoviePy stitching
            # IMPORTANT: Re-initialize long_video_segment_bytes to all None BEFORE generation starts
            st.session_state.long_video_segment_bytes = [None] * len(prompts_to_use) 
            ...
            for i, prompt in enumerate(prompts_to_use):
                segment_status_text.text(f"Generating segment {i+1} of {total_segments} using {st.session_state.long_video_model_choice}...")
                with st.spinner(f"Veo is generating segment {i+1}..."):
                    segment_video_bytes = None
                    if st.session_state.long_video_model_choice == "Veo2":
                        segment_video_bytes = generate_video_with_veo("text", None, prompt) 
                    elif st.session_state.long_video_model_choice == "Veo3":
                        # Corrected call: generate_video_with_veo3 also expects input_type and input_content
                        segment_video_bytes = generate_video_with_veo3("text", None, prompt) 
                    ...
                    if segment_video_bytes:
                        ...
                        if temp_filepath:
                            ...
                            st.session_state.long_video_segment_bytes[i] = segment_video_bytes # Store for individual display
                    else:
                        st.error(f"Failed to generate segment {i+1}. Aborting stitching.")
                        st.session_state.long_video_segments = [] 
                        st.session_state.long_video_segment_bytes = [None] * total_segments # Clear all segment bytes
                        break
    ...
    if st.session_state.long_video_final_video:
        ...
        st.warning("If the final video does not appear, ensure FFmpeg is correctly installed and accessible in your environment.")
    else:
